		...
		var theTime = parseInt(tsecond);
			...
			if(intervalTimeout==0){
				...
				$('#witecontxt').html('正在等待智能合约抽签...');
					...
					$('#modaltext').text('请在弹出的钱包上点击”生成交易“-->”确定“启动本次抽签！');
					ExeConstract("RandDraw","0",'['+$('#currdrowno').text()+']');
				getDrawresult();
			}
}
	...
	var to = dappAddress;
			...
			function returnNuoNuo(resp) {
				...
				if(strresp.indexOf("txhash")!=-1){
					$('#modalprog').css('display','block');
					if(callfun=="RandDraw"){
						$('#modaltext').text('正在进行抽签，如果出现错误提示：The draw has been completed。可不必理会');
					}
					if(callfun=="TakeNo"){
